...
function optionButtons(container, items, current, attr, onPick) {
  ...
  items.forEach((item) => {
    ...
    btn.type = 'button';
    btn.className =
      'rounded-lg border px-3 py-2 text-xs font-semibold transition duration-300 min-h-[40px] ' +
      (current === item.id
        ? 'border-brand-500 bg-brand-50 text-brand-800 dark:border-brand-400 dark:bg-brand-950/50 dark:text-brand-200'
        : 'border-ink-200 bg-white text-ink-700 hover:border-brand-300 dark:border-ink-700 dark:bg-ink-900/60 dark:text-ink-200 dark:hover:border-brand-500');
